...
async function runExperiment4({ assetDir = null } = {}) {
  ...
  const summary = {
    ...
    sensitivitySamples: sensitivityRows.map((row) => ({
      ...
      hypothesisSelection: row.policy.hypothesisSelection,
  ...
  await writeFile(
    ...
    toCsv(summary.sensitivitySamples, [
      ...
      { header: 'mean_frontier_width', value: (row) => row.meanFrontierWidth },
       ...
       { header: 'focus_ratio', value: (row) => row.hypothesisSelection.focusRatio },
       { header: 'ambiguity_ratio', value: (row) => row.hypothesisSelection.ambiguityRatio },
