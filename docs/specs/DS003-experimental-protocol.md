...
## Core Content
The shared data now has three layers. The first layer is the original controlled corpus of eighteen workflow cases across package handling, laboratory sample handling, and manuscript processing. The second layer is an expanded seven-domain benchmark with controlled, paraphrased, and noisy strata, built from thirty-four base cases and rendered as one hundred and two benchmark cases with deterministic development and test splits. The third layer is a novelty set with fourteen unseen or hybrid base traces spanning clinical, legal, quality, logistics, hiring, and explicitly mixed cases. Every benchmark case remains segmented into four ordered textual stages so the same base trace can be evaluated at several **segment depths**.
...
2. **Frontier metrics.** Truth-retention rate asks whether the ground-truth family survives anywhere on the active frontier. Frontier width and domain entropy measure how much structured ambiguity remains. Premature-collapse rate measures how often the correct family disappears while the current top confidence is already high. On the expanded benchmark, per-domain precision, recall, F1, confusion counts, and mean truth rank should also be reported so transfer claims do not hide domain-specific failures.
3. **Questioning metrics.** Information gain is computed as the entropy drop expected from a discriminating question over the current retained-domain distribution. Question availability records whether a useful question still exists. Multi-step recovery metrics include entropy per budget, final accuracy per budget, rescued-from-wrong rate, and adversarial failure mechanics such as entropy reduction per answered question and harmful-question rate.
...
Failure analysis is mandatory. The current implementation must state three limitations plainly in downstream reports. First, the controlled observer and questioning results do not prove full open-ended theory induction. Second, open-set handling currently demonstrates disciplined uncertainty management rather than synthesis of a genuinely new local theory family. Third, multi-step questioning degrades sharply under adversarial answers and should be described as a current weakness rather than hidden behind aggregate scores. Experiment 7 must therefore define adversarial answers explicitly as the fixed worst-branch policy rather than leaving that condition informal.
