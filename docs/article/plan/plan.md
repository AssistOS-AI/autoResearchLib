...
| 3. Formal objects, budgets, and update rules | Define `O(x)`, `T`, `N(O(x))`, `F`, the budget objects, domain-level question entropy, rescue tolerance, alignment utility, and the canonical CNL projection. | `DS002-data-model.md`, `DS001-library-architecture.md`, `DS006-library-usage.md`, `DS010-question-selection.md`, `DS012-cue-provenance.md` | `plan/chapters/ch3-data-model.md` |
...
4. If a chapter plan, any declared dependency, or any file referenced through `{{FILE:...}}`, `{{JSON:...}}`, or `{{JSON_TABLE:...}}` tokens is newer than the generated chapter markdown file, the skill refreshes that chapter markdown file under `plan/chapters/`.
5. The skill reads `plan/bibliography.md` as the editable source of truth for citation metadata, verification mode, bootstrap text, and deterministic support profiles, then validates citation support through `plan/bibliography/<citation-key>/`.
...
7. If `index.html` is older than `plan/plan.md`, `plan/bibliography.md`, any generated chapter markdown file, any article-facing SVG asset, or any bibliography verification artifact, the skill rebuilds the final HTML, emits browser-side print or save-PDF controls, and records visible generation provenance.
