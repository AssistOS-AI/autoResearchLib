...
7. `docs/article/plan/bibliography.md` is the editable bibliography source of truth. `docs/article/plan/bibliography/` stores one folder per cited source with the fetched source cache and the checked-claims cache used during article build. Citation checks must record support snippets and verification status, and any `manual-waived` reference must remain explicitly marked as such rather than silently passing as a normal fetched-source citation.
...
9. The generated article HTML should include browser-side print or save-PDF controls, visible print-limit notes, and generator provenance metadata, but repository documentation and article text must remain honest that browser-managed print headers or footers cannot be fully suppressed through standard web APIs.
10. Article publication is agent-orchestrated. The repository should not present article publication as a generic npm or command-line interface because article review, plan repair, bibliography judgment, and figure revision remain agent responsibilities.
11. The current validation ladder contains seven experiments: structural observer and questioning studies, cue-masking recovery, sensitivity and ablation, expanded benchmark transfer, novelty handling, and multi-step questioning. Documentation should keep those roles distinct and state the current limits honestly.
